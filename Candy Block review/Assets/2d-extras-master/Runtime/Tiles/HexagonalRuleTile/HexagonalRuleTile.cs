...
namespace UnityEngine {
    ...
    public class HexagonalRuleTile : RuleTile {
        ...
        private static readonly Vector3Int[,] PointedTopNeighborOffsets = {
            {
                new Vector3Int(1,  0, 0), new Vector3Int(0, -1, 0), new Vector3Int(-1, -1, 0), new Vector3Int(-1, 0, 0),
                new Vector3Int(-1, 1, 0), new Vector3Int(0, 1,  0)
            }, {
                new Vector3Int(1, 0, 0), new Vector3Int(1, -1, 0), new Vector3Int(0, -1, 0), new Vector3Int(-1, 0, 0),
        ...
        protected override bool RuleMatches (TilingRule rule, ref TileBase[] neighboringTiles,
                                             ref Matrix4x4 transform) {
            // Check rule against rotations of 0, 60, 120, 180, 240, 300
            for (int angle = 0;
                 angle <= (rule.m_RuleTransform == TilingRule.Transform.Rotated ? 300 : 0);
                 angle += 60) {
            ...
            if ((rule.m_RuleTransform == TilingRule.Transform.MirrorX) &&
                RuleMatches(rule, ref neighboringTiles, true, false)) {
            ...
            if ((rule.m_RuleTransform == TilingRule.Transform.MirrorY) &&
                RuleMatches(rule, ref neighboringTiles, false, true)) {
                transform = Matrix4x4.TRS(Vector3.zero, Quaternion.identity, new Vector3(1f, -1f, 1f));
                return true;
            }
        ...
        protected override Matrix4x4 ApplyRandomTransform (TilingRule.Transform type, Matrix4x4 original,
            ...
            switch (type) {
                ...
                case TilingRule.Transform.Rotated:
                    int angle = Mathf.Clamp(Mathf.FloorToInt(perlin * neighborCount), 0, neighborCount - 1) *
                                (360 / neighborCount);
        ...
        protected override void GetMatchingNeighboringTiles (ITilemap tilemap, Vector3Int position,
                                                             ref TileBase[] neighboringTiles) {
